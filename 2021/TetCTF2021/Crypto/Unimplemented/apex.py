...
"""
phi(p**2 * q**2) = phi(p**2)*phi(q**2) = (p**2-p)  	[for norm ints]
phi(p**2 * q**2) = (p**4 - p**2)*(q**4 - q**2)		[for Gaussian Ints]
"""

# phi = (p**4 - p**2)*(q**4 - q**2)			...(1)
phi = (p**3 - p)*(q**3 - q)				# reduced from (1)
...
m = complex_pow(c,d,n)
print(f"Plaintext (real): \n\n{long_to_bytes(m.re)}\n\n")
print(f"Plaintext (imaginary): \n\n{long_to_bytes(m.im)}\n")
flag = unpad(long_to_bytes(m.im))
print(flag)
